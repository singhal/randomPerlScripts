...
sub varFilter {
  my %opts = (d=>2, D=>10000000, a=>2, W=>10, Q=>10, w=>3, p=>undef, 1=>1e-4, 2=>1e-100, 3=>0, 4=>1e-4, G=>0, S=>1000, e=>1e-4, h=>1e-6, A=>undef, M=>undef, b=>undef, r=>undef);
  getopts('pd:D:W:Q:w:a:1:2:3:4:G:S:e:h:A:M:b:r:', \%opts);
  die(qq/
...
Options: -Q INT    minimum RMS mapping quality for SNPs [$opts{Q}]
	 ...
	 -A FILE   ancestral fasta file (should be sorted from lowest to highest contig)
	 -M CHAR   mutation type to remove (ex. 'CT GA') 
	 -b CHAR   outfile name for bed format list of sites
...
\n/) if (@ARGV == 0 && -t STDIN);
  ...
  open(FASTA, '<', $opts{A}) if (defined $opts{A});
  my $line; my @seq; my $base; my $excont; #Tyler added variables
  if ($opts{r}) {
	open(RMV, '<', $opts{r});
	my @contex = <RMV>;
	$excont = join "\t", @contex;
  ...
  while (<>) {
	...
	# Tyler's added code for eliminating sites with certain mutations and exluding contigs
    if ($excont) {
	next if $excont =~ /\b$t[0]\b/;
	}
    if ($opts{A}) {
	...
	if ($type == 1) { #SNP	
		...
		if ($exactp <= $opts{h}) {
			print STDERR "departure from HWE (p=$exactp) $_" if $opts{p};
			next;
		} 	
	} # end HWE SNP filter
	...
	while (@staging) {
	  ...
	  varFilter_aux(shift(@staging), $opts{p}, $opts{b}); #calling function is a bit slower, not much
	}
	my $flt = 0;
	# parse annotations
  ...
  while (@staging) {
	varFilter_aux(shift @staging, $opts{p}, $opts{b});
  }
  close FASTA if (defined $opts{A});
  close BED if ($opts{b});
}

sub varFilter_aux {
  my ($first, $is_print, $out) = @_;
  if ($first->[1] == 0) {
	print join("\t", @$first[3 .. @$first-1]), "\n";
  if (defined $out && defined $$first[4]) {
	my $startpos = $$first[4]-1;
	print BED "$$first[3]\t$startpos\t$$first[4]\n";
	}
  } elsif ($is_print) {
