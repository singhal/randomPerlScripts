...
my $dir = '/media/DataDrive/sutureGenomics/lineageSNPs/lampro/';
my $final = $dir . 'Lampro.dadi.out';
my $vcf = $dir . 'jointLampro.vcf.out';
my $outgroup = $dir . 'lamproOutgroup.mpileup.out';
my $depth1 = $dir . 'Lampro_N.depth.out';
my $depth2 = $dir . 'Lampro_C.depth.out';
my $depth3 = $dir . 'Lampro_S.depth.out';
my $seq = $dir . 'Lampro_N_trinity.fa.final.annotated';
my $cov = '20';
my $qual = '20';
my @seqFiles = </media/DataDrive/sutureGenomics/lineageSNPs/lampro/seqFiles/*annotated>;
my $numSnp = 2;
my $numWin = 1;
...
$var = checkCoverage($depth3,$var);
print "Checked coverage (pass 3) of all the variants!\n";
...
sub makeDadi {
	...
	open(OUT, ">$final");
	print OUT "Lampro\tOutgroup\tAllele1\tNpop\tCpop\tSpop\tAllele2\tNpop\tCpop\tSpop\tGene\tPosition\n";
	my %var = %{$var};
	foreach my $c (keys %var) {
		foreach my $pos (keys %{$var{$c}}) {
			...
			my $a1_3 = 10 - $var{$c}{$pos}{'lin3'};
			print OUT $tri, "\t", $tri, "\t", $var{$c}{$pos}{'ref'}, "\t", $a1_1, "\t", $a1_2, "\t", $a1_3, "\t", $var{$c}{$pos}{'alt'}, "\t",  $var{$c}{$pos}{'lin1'}, "\t",  $var{$c}{$pos}{'lin2'}, "\t", $var{$c}{$pos}{'lin3'}, "\t", $c, "\t", $pos, "\n";
			}
		}
	close(OUT);	
	}
				...
				if ($count >  $numSnp ) {
					...
					if ($snp{$winner} > $numWin) {	
						if ($winner eq $var{$c}{$pos}{'ref'}) {
							...
							$var{$c}{$pos}{'lin3'} = 10 - $var{$c}{$pos}{'lin3'};
							}
						}	
					}
				}	
				...
				if (scalar(keys %snp) >  0 ) {
					...
					if ($snp{$winner} > $numWin) {
						if ($winner eq $var{$d[0]}{$d[1]}{'ref'}) {
							...
							$var{$d[0]}{$d[1]}{'lin3'} = 10 - $var{$d[0]}{$d[1]}{'lin3'};
							}	
						}	
					}	
				}	
